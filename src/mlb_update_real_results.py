...
# def obtener_fechas_ayer():
#     """Calcula las fechas de ayer para la ejecución automática a las 5am"""
#     ayer = datetime.now() - timedelta(days=1)
#     fecha_bref = ayer.strftime("%A, %B %-d, %Y").replace(" 0", " ") 
#     fecha_db = ayer.strftime("%Y-%m-%d")
#     return fecha_bref, fecha_db, ayer.year

# # ============================================================================
# # PROCESO PRINCIPAL: mlb_update_real_results.py
# # ============================================================================
# def actualizar_resultados_reales():
#     fecha_bref, fecha_db, year_val = obtener_fechas_ayer()

#     print(f"🕒 Iniciando actualización de resultados reales para: {fecha_bref}")
...
#             links = cursor.find_all('a')
#             if len(links) >= 3:
#                 away_team_name = links[0].text.strip()
#                 home_team_name = links[1].text.strip()
...
#                     data_temp_resultados.append({
#                         'home_team_real': home_team_name,
#                         'away_team_real': away_team_name,
#                         'ganador': ganador,
#                         'year': year_val,
#                         'fecha': fecha_db,
#                         'score_home': score_home,
#                         'score_away': score_away,
#                         'game_id': box_link # Capturamos el ID aquí
#                     })
#         except Exception as e:
#             print(f"⚠️ Error procesando partido: {e}")
#         cursor = cursor.find_next_sibling()

#     if data_temp_resultados:
#         with sqlite3.connect('../data/mlb_reentrenamiento.db') as conn:
#             df_res = pd.DataFrame(data_temp_resultados)
            
#             # Cruzamos con la tabla lineup_ini para obtener los pitchers exactos
#             query_pitchers = f"SELECT game_id, team, player as pitcher FROM lineup_ini WHERE fecha='{fecha_db}' AND [order]='P'"
#             df_p = pd.read_sql(query_pitchers, conn)
            
#             if df_p.empty:
#                 print("❌ No se encontraron lineups previos para cruzar datos de lanzadores.")
#                 return

#             # Merge para Home Pitcher
#             df_final = df_res.merge(df_p, left_on=['game_id', 'home_team_real'], right_on=['game_id', 'team'], how='left')
#             df_final = df_final.rename(columns={'pitcher': 'home_pitcher'}).drop(columns=['team'])
            
#             # Merge para Away Pitcher
#             df_final = df_final.merge(df_p, left_on=['game_id', 'away_team_real'], right_on=['game_id', 'team'], how='left')
#             df_final = df_final.rename(columns={'pitcher': 'away_pitcher'}).drop(columns=['team'])

#             # Renombrar columnas al formato de entrenamiento
#             df_final = df_final.rename(columns={'home_team_real': 'home_team', 'away_team_real': 'away_team'})
            
#             # MODIFICACIÓN: Se añade 'game_id' a la lista de columnas finales
#             columnas_finales = [
#                 'game_id', 'home_team', 'away_team', 'home_pitcher', 'away_pitcher', 
#                 'ganador', 'year', 'fecha', 'score_home', 'score_away'
#             ]
...
#             # Limpieza y guardado
#             # Mantenemos el dropna en pitchers para asegurar calidad de datos
#             df_export = df_final[columnas_finales].dropna(subset=['home_pitcher', 'away_pitcher'])
            
#             if not df_export.empty:
#                 df_export.to_sql('historico_real', conn, if_exists='append', index=False)
#                 print(f"✅ Se han guardado {len(df_export)} resultados reales con su game_id en 'historico_real'.")
#             else:
#                 print("⚠️ No se pudieron asociar los lanzadores con los resultados reales.")
#     else:
#         print("No se encontraron resultados finales para procesar.")
