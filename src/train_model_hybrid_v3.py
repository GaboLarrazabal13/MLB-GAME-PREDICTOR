...
#     # Manejo de Cache Incremental (Original)
#     X_dict_list = []
#     y_list = []
#     indices_procesados = []
...
#     # --- INTEGRACIÓN DE NEUTRALIZACIÓN (Solución al 60% sin cambiar nombres) ---
#     if 'home_starter_WHIP' in X.columns:
#         X['super_neutralizacion_whip_ops'] = (X['home_starter_WHIP'] * X['away_team_OPS']) - \
#                                              (X['away_starter_WHIP'] * X['home_team_OPS'])
#         X['super_resistencia_era_ops'] = (X['home_starter_ERA'] / (X['away_team_OPS'] + 0.01)) - \
...
#     X_selected = selector.fit_transform(X_scaled, y)
#     cols_seleccionadas = X.columns[selector.get_support()].tolist()
#     print(f"       Features clave: {cols_seleccionadas}")
...
#     acc = accuracy_score(y_test, mejor_modelo.predict(X_test))
#     print(f"\n ENTRENAMIENTO COMPLETADO - ACCURACY FINAL: {acc:.2%}")
    
#     print(f" Guardando modelo en {modelo_path}")
#     mejor_modelo.get_booster().save_model(modelo_path.replace('.pkl', '.json'))
