"""
API REST para Predicción de Partidos MLB
FastAPI + modelo ML entrenado
"""
...
app = FastAPI(
    title="MLB Game Predictor API",
    description="API para predecir resultados de partidos de MLB usando Machine Learning",
    version="1.0.0"
)
...
def extraer_features(home_team, away_team, home_pitcher, away_pitcher, year=2026):
    """Extrae todas las features necesarias para la predicción"""
...
async def root():
    ...
    return {
        "message": "MLB Game Predictor API",
        "version": "1.0.0",
        "endpoints": {
...
def extraer_features_con_stats(home_team, away_team, home_pitcher, away_pitcher, year=2026):
    ...
    # Crear vector de features (igual que antes)
    features = {}
    ...
    # Lanzadores
    if pitcher1_stats:
        features.update({f'home_pitcher_{k}': v for k, v in pitcher1_stats.items() if k != 'nombre'})
    else:
        features.update({
            'home_pitcher_ERA': stats_team1.get('team_ERA_mean', 4.0),
            'home_pitcher_WHIP': stats_team1.get('team_WHIP_mean', 1.3),
    ...
    if pitcher2_stats:
        features.update({f'away_pitcher_{k}': v for k, v in pitcher2_stats.items() if k != 'nombre'})
    else:
        features.update({
            'away_pitcher_ERA': stats_team2.get('team_ERA_mean', 4.0),
            'away_pitcher_WHIP': stats_team2.get('team_WHIP_mean', 1.3),
