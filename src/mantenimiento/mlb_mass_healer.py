...
def get_gaps():
    ...
    c = conn.cursor()
    query = """
    SELECT r.fecha, COUNT(DISTINCT r.home_team) as total_juegos,
           COUNT(DISTINCT p.home_team) as total_preds
    FROM historico_real r
    LEFT JOIN predicciones_historico p ON r.fecha = p.fecha
...
def heal_date(date):
    ...
    # Scraper
    scraper_path = os.path.join(SCRIPTS_DIR, "mlb_daily_scraper.py")
    subprocess.run([sys.executable, scraper_path], env=env)
    # Engine
    engine_path = os.path.join(SCRIPTS_DIR, "mlb_predict_engine.py")
    subprocess.run([sys.executable, engine_path], env=env)


if __name__ == "__main__":
    gaps = get_gaps()
    for date, _, _ in gaps:
        heal_date(date)
